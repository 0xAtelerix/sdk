...
func TestGetBlock(t *testing.T) {
	...
	t.Run("corrupted block value returns error", func(tr *testing.T) {
		const corruptedBlockNumber = 808

		require.NoError(tr, db.Update(tr.Context(), func(tx kv.RwTx) error {
			return tx.Put(
				BlockNumberBucket,
				NumberToBytes(corruptedBlockNumber),
				[]byte("not cbor data"),
			)
		}))

		require.NoError(tr, db.View(tr.Context(), func(tx kv.Tx) error {
			_, err := GetBlock(tx, BlockNumberBucket, NumberToBytes(corruptedBlockNumber), nil)
...
func TestExtractTransactions(t *testing.T) {
	...
	t.Run("decodes transactions", func(tr *testing.T) {
		txs := []testTx{newTestTx(0x01), newTestTx(0x02)}

		block := buildBlock(5, filled(0x05), 555, txs)
		payload := encodeBlock(block)
		require.NotNil(tr, payload)
		...
		for i, tx := range res {
			require.Equal(tr, txs[i].Hash(), tx.Hash())
		}
	})

	t.Run("empty transactions returns empty slice", func(tr *testing.T) {
		block := buildBlock(6, filled(0x06), 666, nil)
		payload := encodeBlock(block)
		require.NotNil(tr, payload)
