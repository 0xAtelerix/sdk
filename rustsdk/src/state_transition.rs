...
use libmdbx::{DatabaseKind, Transaction, RW};
use thiserror::Error;
...
pub trait StateTransitionSimplified<T: AppTransaction, D: DatabaseKind> {
    /// Processes a single transaction and returns a list of external transactions.
    fn process_tx(&self, tx: &T, txn: &mut Transaction<RW, D>) -> Result<Vec<ExternalTransaction>, StateTransitionError>;
    /// Processes an external block and returns a list of external transactions.
    fn process_block(&self, block: &ExternalBlock, txn: &mut Transaction<RW, D>) -> Result<Vec<ExternalTransaction>, StateTransitionError>;
}

/// Trait defining the state transition interface for processing a batch.
pub trait StateTransitionInterface<T: AppTransaction, D: DatabaseKind> {
    /// Processes a batch containing transactions and external blocks,
    /// returning a combined list of external transactions.
    fn process_batch(&self, batch: &Batch<T>, txn: &mut Transaction<RW, D>) -> Result<Vec<ExternalTransaction>, StateTransitionError>;
}
...
impl<S, T, D> StateTransitionInterface<T, D> for BatchProcessor<S>
where
    S: StateTransitionSimplified<T, D>,
    T: AppTransaction,
    D: DatabaseKind,
{
    fn process_batch(&self, batch: &Batch<T>, txn: &mut Transaction<RW, D>) -> Result<Vec<ExternalTransaction>, StateTransitionError> {
        ...
        for external_block in &batch.external_blocks {
            let mut txs = self.simplified.process_block(external_block, txn)?;
            ext_txs.append(&mut txs);
        }

        for tx in &batch.transactions {
            let mut txs = self.simplified.process_tx(tx, txn)?;
            ext_txs.append(&mut txs);
        }
